...
def convert_examples_to_features(code1_tokens,code2_tokens,label,url1,url2,tokenizer,args,cache):
    ...
    if tokenizer.cls_token is None:
       ...
       if tokenizer.pad_token_id is None: 
          code1_ids=code1_ids+[tokenizer.eos_token_id]*padding_length       
       else:       
       ...
       if tokenizer.pad_token_id is None:      
          code2_ids=code2_ids+[tokenizer.eos_token_id]*padding_length       
       else:         
...
def train_GA(args, train_dataset, model, tokenizer,pool):
    """ Train the model """
...
class Chromosome:
    ...
    def __str__(self):
        return (
        f"[Chromosome-{self.id}]\n"
        f"Fitness: {self.fitness}\n"
        f"Structure: {self.describe()}\n"
        f"Tokenizer: {self.get_tokenizer()}"
    )
...
def build_model(chromosome,args):
    ...
    for i, c in enumerate(cfgs):
        ...
        merged_desc.append(
            f"[{c['model_id']}: layer {c['layers']}]"
        )

        if i < len(merged_connectors):
            merged_desc.append(
                f"--({merged_connectors[i]})-->"
            )

    print("Merged Structure:", " ".join(merged_desc))
    ################################
...
def mutate(c, prob=0.3):
    ...
    if random.random() < prob:
        ...
        print(f"\n[BLOCK MUTATION]")
        print(f"Position: {i}")
        print(f"Old Block Index: {old_block}")
        print(f"New Block Index: {c.blocks[i]}")

        print("Old Block Config:")
        print(BLOCK_POOL[old_block])

        print("New Block Config:")
        print(BLOCK_POOL[c.blocks[i]])
    ...
    if random.random() < prob and \
       len(c.connectors) > 0:
        ...
        print(f"\n[CONNECTOR MUTATION]")
        print(f"Position: {i}")
        print(f"Old Connector: {old_conn}")
        print(f"New Connector: {c.connectors[i]}")
        ...
        for gen in range(generations):
            ...
            for c in pop:
                if c.fitness is None:
                   model = build_model(c,args)
                   train_dataset = load_and_cache_examples(args, model.tokenizer, evaluate=False,pool=pool)
                   c.fitness = train_GA(args, train_dataset, model, model.tokenizer, pool) # 适应度函数 - 轻量训练
