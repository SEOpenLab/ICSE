...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          ...
                          RobertaConfig, RobertaModel, RobertaTokenizer,AutoTokenizer,AutoModel,AutoConfig,
...
def main():
    ...
    config = config_class.from_pretrained("./starencoder")
    config.num_labels=2
    tokenizer = prepare_tokenizer(tokenizer_class, args.tokenizer_name, args.do_lower_case, args.cache_dir)
    ...
    print(tokenizer.pad_token, tokenizer.convert_tokens_to_ids(tokenizer.pad_token)) #49152？
    if args.block_size <= 0:
        args.block_size = tokenizer.max_len_single_sentence  # Our input block size will be the max possible for the model
    args.block_size = min(args.block_size, tokenizer.max_len_single_sentence)
    print(config_class, model_class, tokenizer_class)
    if args.model_name_or_path:
        model = model_class.from_pretrained("./starencoder", config=config)    
    else:
        model = model_class("./starencoder", config=config) 
