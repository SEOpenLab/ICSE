...
def main():
    ...
    config = AutoConfig.from_pretrained('./PolyCoder-160M',
    ...
    if args.model_name_or_path:
        model = AutoModel.from_pretrained('./PolyCoder-160M',
                                            from_tf=bool('.ckpt' in args.model_name_or_path),
                                            config=config,
                                            cache_dir=args.cache_dir if args.cache_dir else None)    
    else:
        model = model_class(config)
    # 1. 取出原始 layers（GPTNeoX / PolyCoder）
    original_layers = model.layers  # ModuleList

    # 2. 指定你想要的层（index 从 0 开始）
    selected_indices = [2, 3, 8, 9, 10, 11]
