...
class Model(nn.Module):   
    def __init__(self,  bert, codebert, config,tokenizer,args):
    ...
    def forward(self, input_ids=None,attention_mask=None,labels=None): 
        ...
        bert_outputs = self.bert(input_ids= input_ids,attention_mask=attention_mask)[0]
        # ===== Adapter（对齐）=====
        adapted = self.adapter(bert_outputs) + bert_outputs
        # ===== CodeBERT Encoder=====
        code_outputs = self.codebert(inputs_embeds=adapted,attention_mask=attention_mask)[0]
